...
export class SmartThingsWindFreePlatform implements DynamicPlatformPlugin {
  ...
  public async discoverDevices(): Promise<void> {
    ...
    const exposedDeviceIds = new Set<string>();
  ...
  public get config(): NormalizedConfig {
    return this.normalizedConfig;
  }

  private addOrRestoreAccessory(device: SmartThingsDevice, capabilities: string[], client: SmartThingsClient): void {
